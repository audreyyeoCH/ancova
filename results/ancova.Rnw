...
isScienceFamaly = grepl("^Science$|^Science Signaling$|^Science Translational Medicine$|^Science Advances$|^Science Immunology$", data.scopus$Source.title, ignore.case = TRUE)

idx = !(hasScience == 1 & isScienceFamaly == 0)
idx1 = !(data.scopus$Source.title == "PloS one")
!(data.scopus$Source.title == "PloS one") #74rd journal
!(data.scopus$Source.title == "Information Systems and Evolution")
